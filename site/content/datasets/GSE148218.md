---
title: "GSE148218"
type: docs
weight: 114
geekdocHidden: false
---

# GSE148218

| Field | Value |
|-------|-------|
| **Dataset ID** | `GSE148218` |
| **GSE / GSM accession** | [GSE148218](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE148218) |
| **Modality** | scRNA |
| **Species** | human |
| **Tissue** | tumor |
| **Cell count** | 50,747 |
| **PubMed** | [PMID:33218352](https://pubmed.ncbi.nlm.nih.gov/33218352/) |
| **Submission date** | Apr 07 2020 |
| **Source name (GEO)** | Primary pre-B-ALL BM |

## GEO title

> Single cell characterization of arrested B-lymphoid differentiation and leukemic cell states in ETV6-RUNX1-positive pediatric leukemia

## Description (local manifest)

GSE148218_bmALLHmCancer

---

<small>Record auto-generated from `data/benchmark_manifest.csv` by
`scripts/build_site_data.py`. All fields verified against GEO online
metadata via GEOparse.</small>
