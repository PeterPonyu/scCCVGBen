---
title: "GSE165784"
type: docs
weight: 117
geekdocHidden: false
---

# GSE165784

| Field | Value |
|-------|-------|
| **Dataset ID** | `GSE165784` |
| **GSE / GSM accession** | [GSE165784](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE165784) |
| **Modality** | scRNA |
| **Species** | human |
| **Tissue** | retina |
| **Cell count** | 11,492 |
| **PubMed** | [PMID:35061025](https://pubmed.ncbi.nlm.nih.gov/35061025/) |
| **Submission date** | Jan 29 2021 |
| **Source name (GEO)** | Fibrous membrane_PVR |

## GEO title

> Single-cell RNA-sequencing reveals the heterogeneity of microglia in fibrous membrane derived from proliferative diabetic retinopathy and proliferative vitreoretinopathy

## Description (local manifest)

GSE165784_RetinaHmDev

---

<small>Record auto-generated from `data/benchmark_manifest.csv` by
`scripts/build_site_data.py`. All fields verified against GEO online
metadata via GEOparse.</small>
