---
title: "GSE247719"
type: docs
weight: 133
geekdocHidden: false
---

# GSE247719

| Field | Value |
|-------|-------|
| **Dataset ID** | `GSE247719` |
| **GSE / GSM accession** | [GSE247719](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE247719) |
| **Modality** | scRNA |
| **Species** | mouse |
| **Tissue** | muscle |
| **Cell count** | 696,410 |
| **PubMed** | [PMID:39607904](https://pubmed.ncbi.nlm.nih.gov/39607904/) |
| **Submission date** | Nov 14 2023 |
| **Source name (GEO)** | kidney |

## GEO title

> A Panoramic View of Cell Population Dynamics in Mammalian Aging

## Description (local manifest)

GSE247719

---

<small>Record auto-generated from `data/benchmark_manifest.csv` by
`scripts/build_site_data.py`. All fields verified against GEO online
metadata via GEOparse.</small>
