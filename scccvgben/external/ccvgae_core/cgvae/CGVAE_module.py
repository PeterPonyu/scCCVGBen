...
class BaseGraphNetwork(nn.Module):
    ...
    def _create_conv_layer(self, in_dim: int, out_dim: int, Cheb_k: int, alpha: float) -> nn.Module:
        ...
        # scCCVGBen extensions — GIN and EdgeConv require an internal MLP
        elif self.conv_layer_type == 'GIN':
            mlp = nn.Sequential(
                nn.Linear(in_dim, out_dim), nn.ReLU(), nn.Linear(out_dim, out_dim)
            )
            return self.conv_layer(mlp)
        elif self.conv_layer_type == 'EdgeConv':
            mlp = nn.Sequential(
                nn.Linear(2 * in_dim, out_dim), nn.ReLU(), nn.Linear(out_dim, out_dim)
            )
            return self.conv_layer(mlp, aggr='mean')
        elif self.conv_layer_type == 'SuperGAT':
            return self.conv_layer(in_dim, out_dim, heads=1, dropout=0.0, attention_type='MX')
        # GATv2 and remaining variants use the standard (in, out) signature
    ...
    def _process_layer(self, x: torch.Tensor, conv: nn.Module, edge_index: torch.Tensor,
        ...
        # scCCVGBen extensions — these layers don't accept edge_weight
        elif isinstance(conv, (GINConv, EdgeConv, GATv2Conv, SuperGATConv)):
            return conv(x, edge_index)
