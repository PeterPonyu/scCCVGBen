#!/usr/bin/env python3
"""make_fig04_ccvgae_composite.py — CCVGAE flagship vs the three pair variants.
...
folding the per-variant CCVGAE-vs-X comparisons into a single panel.

Output: ``figures/fig04.{pdf,png}``.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--encoder-dir", type=Path,
                   ...
                   help="Source for the CCVGAE flagship row (default: "
                        "encoder_sweep/, which has full 100-dataset coverage of "
                        "scCCVGBen_GAT at the scCCVGBen training config. The "
                        "pair_sweep ablation columns use the legacy "
                        "epochs=200 / i_dim=10 config, so methodology differs "
                        "between the flagship column and the ablation columns; "
                        "this is documented in fig04 caption.")
    p.add_argument("--manifest", type=Path,
                   default=REPO_ROOT / "data" / "benchmark_manifest.csv")
    p.add_argument("--out-dir", type=Path, default=REPO_ROOT / "figures")
    p.add_argument("--out", default="fig04_ccvgae_composite")
    ...
    display_order = [_VARIANT_LABELS.get(method, method) for method in methods_present]
    ...
    audit = metric_coverage_audit(
        ...
        expected_methods=len(methods_present),
    )
    write_metric_audit(args.out_dir / "_metric_audit.csv", audit, figure_id="fig04")

    fig, _ = create_metric_grid_figure(
        long_df,
        metric_grid=METRIC_PANEL_GRID,
        group_col="variant_display",
        reference_method=_VARIANT_LABELS[_FLAGSHIP_DISPLAY_NAME],
        ...
        per_row_height=3.42,
        xtick_rotation=42,
        xtick_labelsize=12.5,
        hspace=0.50,
        palette=palette_for_methods(display_order),
        significance_marker_fontsize=12.2,
        significance_ns_fontsize=10.0,
        significance_step_fraction=0.158,
