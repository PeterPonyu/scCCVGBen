#!/usr/bin/env python3
"""preprocess_scrna.py — CLI wrapper around scccvgben.data.preprocessing.preprocess_scrna.

Processes each input h5ad in place (overwrites) or writes to a sibling
*_pp.h5ad file when --out-suffix is specified.

Usage:
    python scripts/preprocess_scrna.py file1.h5ad file2.h5ad ...
    python scripts/preprocess_scrna.py workspace/data/scrna/*.h5ad --out-suffix _pp
"""
...
def main() -> None:
    ...
    for path_str in args.h5ad_paths:
        ...
        log.info("Preprocessing scRNA: %s", path)
        try:
            ...
            log.info("Written -> %s  (%d cells x %d genes)", out_path, adata.n_obs, adata.n_vars)
